...
public static ITestResult testResult;
     ...
     */
    protected String username = "";//"troy2012";
    /**
     * The access key to use when performing HTTP requests to the Sauce REST API.
     */
    protected  String accessKey = "";//a2226d54-263d-488a-9ba4-2dd8c436e503";
    ...
    /**
     * Constructs a new instance of the SauceREST2 class.
     *
    ...
    public void downloadLog(String jobId, String location) {
        ...
        try {
            ...
            logger.log(Level.WARNING, "Error constructing Sauce URL", e);
            System.out.println("REST URL"+String.format(DOWNLOAD_LOG_FORMAT, username, jobId));
        }
        downloadFile(jobId, location, restEndpoint);
    }
     ...
     * @param restEndpoint the URL to perform a HTTP GET
     */
    public void downloadFile(String jobId, String location, URL restEndpoint) {
        ...
        try {
          ...
          //  addAuthenticationProperty(connection);
           ...
           // String saveName = testResult.getName() +format.format(new Date());
            String saveName = jobId + format.format(new Date());
            if (restEndpoint.getPath().endsWith(".flv")) {
                saveName = saveName + ".flv";
            } else if(restEndpoint.getPath().endsWith(".log")){
                saveName = saveName + ".log";
            } else {
            	saveName = testResult.getName() +format.format(new Date());
            }
    ...
    private static void closeInputStream(HttpURLConnection connection) {
        try {
            if (connection != null) {
